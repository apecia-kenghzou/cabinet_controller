...
port.on("open",  async () => {
  ...
  for (let i = 0x00; i <= 0xFF; i++) {
    const frame = buildSerialFrame([i]);
    const hexStr = frame.toString("hex").match(/.{1,2}/g).join(" ");
    console.log(`📤 Sending frame for data 0x${i.toString(16).padStart(2, "0")}: ${hexStr}`);

    // Write frame to serial port
    port.write(frame, (err) => {
      if (err) console.error(`❌ Error sending 0x${i.toString(16)}:`, err.message);
    });
